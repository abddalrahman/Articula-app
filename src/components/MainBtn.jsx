import React, { useContext } from 'react'
import { NavLink } from 'react-router'
import { ChangeLangContext } from '../App'

const MainBtn = ({link_info}) => {

	const {languageIs} = useContext(ChangeLangContext)

	return (
		<>
			{
				link_info.btn_type
				?
					<button disabled={link_info.disabledIs ? link_info.disabledIs : false} className={link_info.link_classes}>
						<span>{link_info.content}</span>
						{
							link_info.icon_classes !=null
							?
								<i className={link_info.icon_classes}></i>
							:
								''
						}

						{
							link_info.img
							?
							<svg className={languageIs =='ar'? 'r-180' : ''} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M20.5816 11.3456L4.74585 2.47758C4.61262 2.40297 4.4598 2.37081 4.30781 2.38538C4.15582 2.39996 4.0119 2.46057 3.89528 2.55913C3.77866 2.65769 3.6949 2.7895 3.65519 2.93694C3.61549 3.08437 3.62173 3.24042 3.67309 3.38421L6.66006 11.7477C6.71832 11.9109 6.71832 12.0891 6.66006 12.2522L3.67309 20.6158C3.62173 20.7596 3.61549 20.9156 3.65519 21.063C3.6949 21.2105 3.77866 21.3423 3.89528 21.4408C4.0119 21.5394 4.15582 21.6 4.30781 21.6146C4.45981 21.6292 4.61263 21.597 4.74585 21.5224L20.5816 12.6544C20.6979 12.5892 20.7948 12.4943 20.8622 12.3792C20.9296 12.2642 20.9652 12.1333 20.9652 12C20.9652 11.8667 20.9296 11.7358 20.8622 11.6207C20.7948 11.5057 20.6979 11.4108 20.5816 11.3456V11.3456Z" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
								<path d="M6.75 12H12.75" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
							</svg>
							:
							""
						}
					</button>
				:
					<NavLink className={link_info.link_classes} to={link_info.link_to}>
						{link_info.content}
						{
							link_info.icon_classes !=null
							?
							<i className={link_info.icon_classes}></i>
							:
							''
						}

						{
							link_info.img
							?
							<svg className='ms-2' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M20.5816 11.3456L4.74585 2.47758C4.61262 2.40297 4.4598 2.37081 4.30781 2.38538C4.15582 2.39996 4.0119 2.46057 3.89528 2.55913C3.77866 2.65769 3.6949 2.7895 3.65519 2.93694C3.61549 3.08437 3.62173 3.24042 3.67309 3.38421L6.66006 11.7477C6.71832 11.9109 6.71832 12.0891 6.66006 12.2522L3.67309 20.6158C3.62173 20.7596 3.61549 20.9156 3.65519 21.063C3.6949 21.2105 3.77866 21.3423 3.89528 21.4408C4.0119 21.5394 4.15582 21.6 4.30781 21.6146C4.45981 21.6292 4.61263 21.597 4.74585 21.5224L20.5816 12.6544C20.6979 12.5892 20.7948 12.4943 20.8622 12.3792C20.9296 12.2642 20.9652 12.1333 20.9652 12C20.9652 11.8667 20.9296 11.7358 20.8622 11.6207C20.7948 11.5057 20.6979 11.4108 20.5816 11.3456V11.3456Z" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
								<path d="M6.75 12H12.75" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
								</svg>
							:
							""
						}
					</NavLink>

			}
		</>
	)
}
 
export default MainBtn
